"""
quick script that prints network statistics for various models (memory, FLOPS, fwdbwd time, etc)
"""
...
from bootstrap.lib.overwrite_print import print

# ------------------------------------------------------------------------------

# globals used in counting
count_ops = 0
count_params = 0
...
def net_metrics(model, data=None, shape=None, nf=10):
    ...
    return count_ops, count_params, dt
...
if __name__ == '__main__':
    print('-'*80)
    from .cifar10 import PNASNet
    net = PNASNet()
    nops, nparams, dt = net_metrics(net, shape=[3, 32, 32])
    print("%s: ops: %.2fM, params: %.2fM fwd dt: %.2fs" % ('PNASNet', nops/1e6, nparams/1e6, dt))
    print('trainable params:', count_trainable_parameters(net))
